...
# YOLOv9 Encoder–Decoder
def yolo_v9_seg(input_shape=(512, 512, 3), num_classes=3):
    ...
    # ---------------- Enhanced Bottleneck (YOLOv9-like) ----------------
    b1 = Conv2D(1024, 3, activation='relu', padding='same')(p4)
    b2 = Conv2D(1024, 3, activation='relu', padding='same')(b1)
    b = Concatenate()([b1, b2])  # feature reuse (YOLOv9 concept)
    ...
    model = Model(inputs=inputs, outputs=out, name='YOLOv9_Segmentation')
    return model


def Model_YOLOv9(Images, Groundtruth, Epoch):
    N = Images.shape[0]
    ...
    model = yolo_v9_seg(input_shape=(IMG_SIZE, IMG_SIZE, 3), num_classes=num_classes)
    model.summary()
    ...
    checkpointer = ModelCheckpoint('yolov9_best.keras', save_best_only=True, verbose=1)
